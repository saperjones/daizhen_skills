---
name: dz_spec
description: An all-in-one spec-writing workflow. Any act of writing a spec / design doc / requirements doc runs this four-stage pipeline — first a prd_design-style multi-round dialogue to pin down requirements, then brainstorming to explore and review the design, then relentless grilling that walks the decision tree until every branch reaches shared understanding (the final question always covers test methodology, laying out test scenarios for the user to confirm), finally producing a single design spec. Everything — this skill's conversation and all outputs — is in English. Triggers when the user input starts with /dz_spec or /daizhen:dz_spec, or expresses intent to "write a spec", "write a design doc", "do requirements design", "design this feature", "turn my requirements into a design", "grill my design", "stress-test this plan", "grill me", etc.
---
